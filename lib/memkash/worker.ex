defmodule Memkash.Worker do
  use Connection
  require Logger
  alias Memkash.Serialization.Opcode
  alias Memkash.Serialization.Header
  alias Memkash.Serialization
  ...
  def init(args) do
    host =
      case Keyword.get(args, :host) do
        host_arg when is_binary(host_arg) -> String.to_charlist(host_arg)
        host_arg -> host_arg
      end
  ...
  def handle_cast({:request, from, header, key, body, extras}, %State{socket: socket, timeout: timeout} = state) do
    ...
    case :gen_tcp.send(socket, bytes) do
      :ok ->
        if not Opcode.quiet?(header.opcode), do: do_receive(socket, from, timeout)
